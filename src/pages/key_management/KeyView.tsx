...
import { Key } from '../../store/slice/editForm.slice';
...
const KeyView = () => {
  ...
  useEffect(() => {
    if (!equipId) return;
    DbAssets.getKeyById(equipId).then((snapshot) => {
  ...
  const {
    ...
    queryKey: [REACT_QUERY_KEYS.KEY_ALLOCATION, equipId],
                ...
                data.map((keyAlloc) => {
                  return (
                    ...
                    >
                      ...
                      <td className="align-top px-4 py-2 text-start">
                        {formatDate(keyAlloc.KeyAllocationStartTime)}
                      </td>
                      <td className="align-top px-4 py-2 text-start ">
                        {formatDate(keyAlloc.KeyAllocationEndTime)}
                      </td>

                      {keyAlloc.KeyAllocationIsReturned &&
                      keyAlloc?.KeyAllocationReturnedAt ? (
                        <td className="align-top px-4 py-2 text-end ">
                          {formatDate(keyAlloc?.KeyAllocationReturnedAt)}
                        </td>
                      ) : (
                        <td
                          onClick={() => {
