...
interface UpdateCalloutStatusModalProps {
  opened: boolean;
...
const UpdateCalloutStatusModal = ({
  opened,
  setOpened,
  empId,
  calloutId,
  ...
  const updateTime = async () => {
    try {
      ...
      await DbShift.updateCalloutStatus({
        calloutId,
        empId,
        field,
        updatedTime,
      ...
      showSnackbar({
        message: `Callout ${field === 'end_time' ? 'end time' : 'start time'} updated successfully`,
        type: 'success',
      });
...
export default UpdateCalloutStatusModal;
